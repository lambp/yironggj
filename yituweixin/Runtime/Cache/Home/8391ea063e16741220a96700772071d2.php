...
<script>
    ...
    <p><span style="color:#F00">修改内容</span> <a href="/?m=admin&a=news" target="mainFrame"'>返回</a></p>
	<p></p>
	<center>
	<form action='/?m=admin&a=update_news' method='post' onsubmit='return checkForm()' enctype="multipart/form-data" id="form1">


        <table width="80%" border="0" cellspacing="0" cellpadding="0" class="tableborder">
			<tr>
			<td colspan=2>
                <center><?php echo ($about["title"]); ?><input type='hidden' value='<?php echo ($about["id"]); ?>' name='id'></center>
            </td>
		  </tr>
            <?php if(''!=$about[image]){ ?>
            <tr>
                <td>图片:</td>
                <td><img src='/images/about/m_<?php echo ($about["image"]); ?>' width=200></td>
            </tr>
            <?php } ?>
            <tr>
                <td>更新图片:</td>
            ...
            <tr>
                <td>概要:</td>
                <td><textarea type='text' id='summary' rows=4 cols=80 name='summary' ><?php echo ($about["summary"]); ?></textarea></td>
            </tr>
		  <tr>
			<td>内容:</td>
